...
def main():
    ...
    parser.add_option("-a", "--all", action="store_true", help="Use all cells per cell types")
    (options, args) = parser.parse_args()

    num_per_type = int(args[0])
    num_per_cluster = int(args[1])
    ...
    datasets, cell_ids, data_matrix = _pure_cluster_dataset(num_per_type, num_per_cluster, targ_genes)
...
def _pure_cluster_dataset(num_per_type, num_per_cluster, targ_genes):
    np.random.seed(8)
    all_cell_ids = []
    all_datasets = []
    data_matrix = None
    ...
    for cell_type, root in DATA_SET_TO_LOCATION.items():
        ...
        mat = np.array(mat[:,tenx_gene_indices])

        print('Subsampling and aggregating...')
        for i in range(num_per_type):
            # Sample 'cluster'
            rand_indices = np.random.choice(all_indices, num_per_cluster, replace=False)
